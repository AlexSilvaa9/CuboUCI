...
-- 3 medicamentos que causan alergias y su gasto total
SELECT 
    {[Measures].[Cantidad]} ON COLUMNS, 
    FILTER(
...
-- 5 Mostrar los tres tratamientos con mayor tasa por fuente de admisión
SELECT 
    {[Measures].[Tasa Infusion]} ON COLUMNS, 
    NONEMPTY(
        GENERATE(
            [Ingreso UCI].[Fuente Admision].MEMBERS, 
            TOPCOUNT(
                NONEMPTYCROSSJOIN(
...
-- 9 cantidad de cada medicamento usado para hombres y para mujeres
WITH 
